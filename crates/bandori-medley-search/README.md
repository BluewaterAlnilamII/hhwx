...
The search combines joint three-team allocation bounds, forward/backward tables for batch destination pruning, reversible ownership partitions, bounded improvements and short-lived exhaustive blocks. Conditional bounds are shared down the DFS path; numeric uncertainty or insufficient working memory never discards a candidate. Tiny searches are checked against an independent exhaustive oracle. The storage budget covers rows/indexes, the score cache, joint workspace reservations and all live conditional tables. Input/model data, the individual-bound index/undo, traversal and chart-sized scoring scratch remain separately accounted. See [the search design](../../documents/bandori-medley-search.md) for proof and resource semantics.
