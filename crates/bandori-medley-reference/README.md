...
This crate is the transparent oracle for one already selected five-card team per song. It validates the fixed input, evaluates Bestdori's 120 equiprobable first-five skill orders, applies the leader again at the sixth trigger, propagates PERFECT/GREAT skill state, and returns an IEEE-754-bit trace. Known native-client differences remain documentation-only.
