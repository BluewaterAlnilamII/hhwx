...
test("the Pixi stage loads the selected stage, point-note atlases, and bounded hit effects", async () => {
  ...
  assert.match(runtime, /const startedMediaTime = await playBandoriMediaElement\(audio, operation\.signal\);[\s\S]*operation\.throwIfSuperseded\(\)/u);
  assert.match(runtime, /const next = playBandoriChartTransport\(transportRef\.current\);[\s\S]*await seekAudioAndTransport\(audio, next\)/u);
  assert.match(runtime, /Math\.min\(currentTransport\.durationSeconds, audio\.currentTime\)/u);
  assert.match(runtime, /frozenMediaTimeRef\.current = requestedTimeSeconds/u);
  assert.match(runtime, /startedTimeSeconds === null\)[\s\S]*pauseAudioInternally\(audio\)/u);
  assert.match(runtime, /onWaiting=\{\(event\) => handleMediaWaiting\(event\.currentTarget\)\}/u);
  assert.match(runtime, /onPlaying=\{\(event\) => handleMediaPlaying\(event\.currentTarget\)\}/u);
  assert.match(runtime, /onError=\{\(event\) => handleMediaError\(event\.currentTarget\)\}/u);
  assert.match(runtime, /onEnded=\{\(event\) => \{[\s\S]*cancelPendingMediaOperation\(\)/u);
  assert.match(runtime, /subscribeContextState\(\(state\) =>/u);
  assert.match(runtime, /audio\.readyState >= MEDIA_HAVE_FUTURE_DATA/u);
  ...
  assert.match(runtime, /const request = seekAudioAndTransport\(audio, requested\);[\s\S]*loopSeekPromiseRef\.current = request[\s\S]*loopSeekPromiseRef\.current !== request/u);
