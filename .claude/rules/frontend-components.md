...
- Neutral inputs and buttons opt into `hhwx-control`, solid information actions use `hhwx-action-accent`, and floating menus, tooltips and dialogs use `hhwx-floating-surface` where applicable. Control selection consumes selection roles independently of information, actions and keyboard focus, including its hover state. Remove competing local appearance classes when a shared class owns that role. Keep geometry independent of color scheme; temporary dark color overrides do not establish a second component design. Inverse text consumes a foreground role, never a content-surface token.
