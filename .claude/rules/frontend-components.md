...
- Neutral inputs and buttons opt into `hhwx-control`, solid blue operations use `hhwx-action-accent`, and floating menus, tooltips and dialogs use `hhwx-floating-surface` where applicable. Control selection consumes selection roles independently of information, actions and keyboard focus, including its hover state. Use paired action background/foreground roles for solid buttons; never borrow status text, border or passive progress colors as their fill. Draggable media ranges share the play button's action fill; passive progress uses information roles. Focus indicators consume focus roles. Remove competing local appearance classes when a shared class owns that role. Keep geometry independent of color scheme; site UI colors switch through theme roles, not local `dark:` colors. Preserve domain-owned artwork, band/chart colors and fixed media-overlay ink as documented exceptions. Inverse text consumes a foreground role, never a content-surface token.
