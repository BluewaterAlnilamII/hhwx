...
- 如果接口需要维持旧协议，不要在同一路由里混用两套结构；应明确标注为历史兼容接口，并把新旧协议差异收敛在边界层。
...
- 对同一实体，数据库层名称、Row 类型名称、DTO 名称应同根可追溯，避免同义多名。示例：数据库表 `gbp_events`→ Row 类型 `EventRow`→ DTO `BandoriEventRecord`，三者均围绕 event 核心词，层次通过后缀区分。