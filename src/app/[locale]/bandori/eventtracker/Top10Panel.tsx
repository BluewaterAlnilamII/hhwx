...
export function Top10Panel({
  ...
  return (
    <div className="relative">
      {isBlockingLoading && (
        <div className="absolute inset-0 z-30 flex items-center justify-center rounded-xl bg-white/75 dark:bg-[#0C111C]/75">
          <div className="flex flex-col items-center">
            <div className="h-10 w-10 animate-spin rounded-full border-4 border-blue-500 border-t-transparent" />
            <p className="mt-4 text-sm font-semibold text-blue-600 dark:text-blue-300">正在获取最新数据...</p>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="mb-3 flex items-center justify-between gap-3 rounded-xl border border-red-200/80 bg-red-50/80 px-3 py-2 text-sm text-red-600 dark:border-red-400/25 dark:bg-red-500/10 dark:text-red-200">
          <span className="font-semibold">TOP10 追踪数据加载失败</span>
          <button
            type="button"
            onClick={refresh}
            className="shrink-0 rounded-full border border-red-200 bg-white px-3 py-1 text-xs font-bold transition hover:border-red-300 hover:bg-red-50 dark:border-red-400/30 dark:bg-slate-900 dark:hover:bg-slate-800"
          >
            重新加载
          </button>
        </div>
      ...
      {view && view.players.length > 0 ? (
        <>
          <div className="mt-3 flex flex-wrap justify-center gap-x-4 gap-y-2 px-1" aria-label="TOP10 玩家图例">
            {view.players.map((player) => (
              <div key={player.uid} className="flex min-w-0 items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300">
                <span
                  aria-hidden="true"
