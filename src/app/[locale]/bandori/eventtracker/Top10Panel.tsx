...
export function Top10Panel({
  ...
  return (
    <div className="relative">
      {isBlockingLoading && (
        <div className="absolute inset-0 z-30 flex items-center justify-center rounded-xl bg-[var(--theme-color-surface-background)]/75">
          <div className="flex flex-col items-center">
            ...
            <p className="mt-4 text-sm font-semibold text-[var(--theme-color-feedback-info-foreground)]">正在获取最新数据...</p>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="mb-3 flex items-center justify-between gap-3 rounded-xl border border-[var(--theme-color-feedback-error-border)] bg-[var(--theme-color-feedback-error-background)] px-3 py-2 text-sm text-[var(--theme-color-feedback-error-foreground)]">
          <span className="font-semibold">TOP10 追踪数据加载失败</span>
          <button
            type="button"
            onClick={refresh}
            className="shrink-0 rounded-full border border-[var(--theme-color-feedback-error-border)] bg-[var(--theme-color-control-background)] px-3 py-1 text-xs font-bold transition hover:bg-[var(--theme-color-feedback-error-background)]"
          >
            重新加载
          </button>
        </div>
