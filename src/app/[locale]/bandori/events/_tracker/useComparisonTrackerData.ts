...
export function useComparisonTrackerData({
  ...
  const comparisonLines = useMemo(() => {
    ...
    return configs.filter(isResolvedConfig).map((config, index) => {
      ...
      const target = buildComparisonTargetMeta(
        config,
        eventMap,
        monthlyOptionMap,
        config.targetType === "monthly"
          ? t("monthlyTarget", { targetId: config.targetId })
  ...
  }, [alignment, configs, currentEnd, currentStart, dataByKey, enabled, eventMap, hasLiveAccess, liveDataByKey, loadingByKey, monthlyOptionMap, server, t]);
