...
export default function BandoriEventSwitcher({
  ...
  return (
    <div className="hhwx-panel relative z-20 grid grid-cols-1 gap-6 border p-4 sm:p-8 xl:grid-cols-[minmax(0,1fr)_minmax(320px,420px)] xl:items-center xl:gap-10 dark:border-gray-800 dark:bg-[#131A2B]">
      <div className={cn(
                  ...
                  >
                    <BandoriServerIcon
                      ...
                      className={cn(
                        ...
                        active && "scale-105 ring-2 ring-[var(--theme-color-control-ring-pressed)] ring-offset-1 ring-offset-[var(--theme-color-panel-background)] dark:ring-offset-slate-900",
                      )}
                    />
                  </button>
                );
        ...
        <div className={cn("min-h-13", server !== undefined && "sm:col-start-2 sm:row-start-1")}>
          ...
          {!showSkeleton ? (
            <div className="grid w-full max-w-[492px] grid-cols-[minmax(0,1fr)_44px_44px] items-center gap-2">
              <select
                className="min-w-0 cursor-pointer truncate rounded-xl border border-[var(--theme-color-control-border-accent)] bg-[var(--theme-color-control-background)] px-4 py-2.5 text-sm font-bold text-[var(--theme-color-text-default)] shadow-xs outline-hidden transition-colors focus:border-[var(--theme-color-control-border-accent)] focus:ring-2 focus:ring-[var(--theme-color-control-border-accent)] dark:border-gray-700/50 dark:bg-[#0C111C] dark:text-gray-300 dark:hover:bg-gray-800 dark:focus:border-[var(--theme-color-control-border-accent)] dark:focus:ring-[var(--theme-color-control-border-accent)]"
              ...
              <Dialog.Root open={isPickerOpen} onOpenChange={setIsPickerOpen}>
                ...
                <Dialog.Portal>
                  ...
                  <Dialog.Content className="fixed left-1/2 top-1/2 z-101 flex max-h-[82vh] w-[min(32rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl bg-[var(--theme-color-floating-background)] shadow-[var(--theme-shadow-floating)] animate-in fade-in zoom-in-95 duration-200 dark:bg-[#131A2B]">
