...
export function SimulatorAdjustmentValue({
  ...
  return (
    <output
      aria-label={ariaLabel}
      aria-live="polite"
      className="inline-flex h-10 min-w-[3.75rem] shrink-0 items-center justify-center rounded-xl border border-[var(--theme-color-border-subtle)] bg-[var(--theme-color-control-background-muted)] px-2 text-base font-black tabular-nums text-[var(--theme-color-text-default)] sm:min-w-24 sm:px-4"
    >
      {children}
    </output>
  );
