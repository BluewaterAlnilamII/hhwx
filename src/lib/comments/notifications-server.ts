...
function toNotification(row: CommentNotificationRow): CommentNotification {
  ...
  if (row.type === "comment_reply") {
    const activityCommentId = row.activity_comment_id ?? row.comment_id;
    return {
      ...base,
      type: row.type,
      activityCommentId,
      reactionEmojiKey: null,
      linkCommentId: activityCommentId,
    };
  }
  ...
  return {
    ...
    reactionEmojiKey: row.reaction_emoji_key ?? "",
    linkCommentId: row.comment_id,
  };
}
...
export async function listCommentNotifications(options: {
  ...
  cursor?: string | null;
}): Promise<CommentNotificationListResponse> {
  const client = createServerSupabaseClient();
