...
| Weapon behavior parity | in progress | Recreate data-driven weapon fire cycles before attempting per-weapon subclass behavior | Current slice simulates every `fireBullets` entry on the selected weapon level, maps `WeaponLevelData.BulletShooterID`, `MinionCount`, and usable `spawnMinionData` into the runtime DTO, and honors run-time weapon leveling from EXP and level-up items, including CN weapon `31` switching from shooter `311` to `312` after a level-up pickup and CN weapon `32` switching an existing summoned minion from AI `110` / shooter `15000` to AI `111` / shooter `15001` after level-up plus locking the level `1` through `8` AI/shooter/taunt-size chain (`110`-`117`, `15000`-`15007`, size `500`-`850`), weapon-level attribute changes, self buffs, first-pass shield/counter player-damage protection buffs such as CN weapon `23` / buff `5`, weapon `25` / buff `6` / bullet `27`, and CN AI `44` hostile bullet `99`, first-pass stealth self-buff attribute modifiers and buff-level fire bullets such as CN weapon `29` / buff `8` / bullet `15`, first-pass minion weapons including CN weapon `16` / minion `2` / bullet `22`, CN weapon `19` / minion `6` / bullet `29`, CN weapon `22` / minion `3` / zero-speed ray bullet `33`, CN weapon `26` / minion `4` / bullet `34`, minion AI state shooters, AI timeline `FireAllWeaponNow`-gated minion weapon fire, group timing including CN weapon `20` / `GroupCount = 4` / `FireGroupCD = 5`, damage judge delay/CD, once-per-enemy vs multi-hit rules, target-driven upgraded fireball fan behavior for CN weapon `1` / bullet `11`, player-only moving multi-bullet direct weapons including CN weapon `2` / bullet `2`, weapon `5` / bullet `5`, weapon `6` / bullet `16`, and weapon `9` / bullet `14` with player-facing targetless base direction, first-pass weapon `5` / `暗夜法球` homing, first-pass weapon `6` / `守护之歌` player orbit, target-driven zero-speed ray behavior for CN weapon `3` / bullet `3` requiring an enemy target, target-driven long-lifetime projectile behavior for CN weapon `7` / bullet `17`, target-driven dual direct bullets for CN weapon `8` / bullet `15` without artificial entry-angle spread, zero-speed self-centered direct fields such as weapon `4` / bullet `4` and weapon `10` / bullets `13` and `12`, `DamageJudgeType = None` force-only overlap behavior for weapon `11` / bullet `6`, CN weapon `11` cardinal force bullets `7` through `10`, CN weapon `12` owner-forward ray bullet `19`, CN weapon `13` owner-forward rect bullet `20`, CN weapon `14` targetless owner-forward ray bullet `18`, CN weapon `15` targetless freeze field bullet `21`, CN weapon `17` targetless stun field bullet `23`, CN weapon `18` direct DOT bullet `28`, CN weapon `21` direct black-hole inward force bullet `31`, CN weapon `24` targetless instant/delayed field bullets `25` and `26`, CN weapon `27` direct friendly buff bullet `32` plus allied-minion overlap, CN weapon `28` level `1`/`8` direct DOT bullet `5` plus shooters `2`/`9`, CN weapon `28` shooter `2` frame-30 all-direction radial bullet `24`, CN weapon `30` and weapon `33` targeted zero-speed direct field bullet `60` alongside shooter timelines, CN weapon `30` shooter `301` same-frame main bullet `60` plus buff bullet `99`, CN weapon `30` shooter `301` and weapon `33` shooter `321` loop interval/lifetime behavior, CN weapon `31` shooter `311` formation-type-3 owner-facing offset, CN weapon `33` shooter `321` enemy slow movement plus friendly buff bullets, `BulletShooterData.BehaviorType = 1` owner-position following, `IsFollowOwnerDirection` owner-facing sync, CN AI `44` hostile ray segment damage for bullet `99`, basic collider shapes, bullet component rotate types, enemy vs friendly hit targets, first-pass allied-minion friendly hit buffs, weapon-shooter friendly hit buffs such as weapon `30` / shooter `301` / buff `109`, weapon-shooter direction-offset behavior such as weapon `28` / shooter `2`, bullet force fields, hit buffs, first-pass taunt hit-buff source targeting, player-side and hostile on-destroy event bullets including parent source-modifier inheritance, bullet boundaries, and equip-driven bullet modifiers; exact `Weapon_XX` custom logic, exact native homing turn rate, exact orbit radius/angle speed, deeper native stealth/taunt semantics, native moving direct-fire formation/angle semantics, and remaining native shooter formation/direction details remain pending |
| Active skill parity | in progress | Recreate character active skill charge and timeline effects from CN `activeSkillData` and `BulletShooterData` | Current slice maps skill levels and timeline events, exposes an active skill input, applies `AddBuffDatas` to player-side targets including first-pass active skill `15` level `1`/`2`/`3` `TargetType = 1` buffs on existing minions and same-frame newly spawned minions, lets those minion-side buffs affect direct minion weapon/AI bullet attack, count, speed, size, lifetime, cooldown, and crit modifiers, spawns first-pass minion entities from `SpawnMinionData`, handles first-pass ring summon formation, lets active-skill minions fire assigned `WeaponID` bullets including `Anon Phantom` minion `5` using weapon `28` level `8`, spawns first-pass active-skill bullet shooters from `BulletShooterID` with player-side source modifier snapshots, covers the active-skill shooter direction values observed in CN data (`0`, `1`, `2`, `3`), locks first-pass `SpawnPos = 3` nearest-enemy shooter placement, locks CN active skill `110` level `1`/`2`/`3` shooters `6000`/`6001`/`6002` loop interval, lifetime, owner-following, and `UIefx_flash_starlight` effect behavior, covers active skill `13` / shooter `13000` four-fireball fan spread plus zero-speed snow-field bullet, and now locks that shooter as `LifeTime = 60` with non-looping snow-field bullet `21` plus looping fireball bullet `11` at `LoopFrameInterval = 15`, covers active skill `14` frame-1 player-side critical buff `9` snapshot into stun field shooter `3000` plus frame-90 delayed damage shooter `3001`, records active-skill full-screen effect names as short-lived serializable `fullScreenEffects` state including CN skill `14` frame-90 `UIefx_flash_song`, renders those events as a first-pass full-screen Phaser flash, covers active skill `16` / shooters `9000`/`9001`/`9002` delayed DokiDoki stun-field level switch, covers active skill `114` / shooter `1001` as a non-following static damage field with `BehaviorType = 0` and `IsFollowOwnerDirection = 0`, covers active skill `116` / shooter `10000` owner-forward star-map field plus EXP/coin pickup-gain buffs, covers active skill `117` level `1`/`2`/`3` / shooter `11000` friendly invincible hit buff `108` on the player plus an overlapping allied minion, locks active skill `111` level `1`/`2`/`3` formation-2 summon into minion AI state transition shooters `14000`/`14001`/`14000`, locks active skill `112` same-event shooter `7000` frame `1`/`3`/`7` bullets plus minion `8` and first-pass minion AI shooter `7003` including its direction-`0` zero-offset fallback, locks active skill `112` level-2/3 same-frame summon groups with AI `205`/`206` and `205`/`206`/`207`, including minion AI shooters `7004`/`7005`, `LoopFrameInterval = 7`, and zero-offset bullets `69`/`70`, locks first-pass `AIStateType = 22` minion orbit for active skill `113` / AI `201` and the active skill `112` floating cannon AI states `205`/`206`/`207`, covers large multi-bullet radial spread, and locks active skill `12` level `1`/`2`/`3` first-pass heal-percent, invincible, and single-player revive buffs; native full-screen VFX/timing beyond short-lived serialized effect-name events and approximate flash rendering, remaining native shooter modifier timing beyond the local same-frame snapshot cases, remaining native shooter formation/direction behavior, deeper native summon formation/AI beyond the covered first-pass AI shooter chains, exact native state-22 class naming/constants, native ally revive targeting, and broader ally targeting beyond player-plus-minion overlap remain pending |
...
| Parity harness | in progress | Scripted comparisons against CN dump-derived rules and retained fixture runs | Current slice has focused local settlement tests plus a CN snapshot-derived active-shooter, active-shooter loop/lifetime, weapon-level-shooter, weapon level-up shooter switching for CN weapon `31` shooter `311` -> `312`, weapon `32` spawn-minion AI/shooter chain for levels `1` through `8` (`110`-`117`, `15000`-`15007`, taunt bullet size `500`-`850`) plus level-up switching from AI `110` -> `111` / shooter `15000` -> `15001`, weapon-shooter formation-type-3 owner-facing offset, weapon-shooter direction-offset, weapon-shooter all-direction radial event, weapon `30` friendly hit-buff shooter including same-frame shooter-main bullet `60`, allied-minion overlap, and shooter `301` loop/lifetime, weapon `33` enemy-slow plus friendly-buff shooter including shooter `321` loop/lifetime, weapon `33` shooter `321` `BehaviorType = 1` owner-position follow and `IsFollowOwnerDirection` owner-facing sync, weapon `1` level-2 targeted two-shot fireball fan, weapon `2`, `5`, `6`, and `9` targetless moving multi-bullet cases with player-facing targetless direction, weapon `5` Dark Orb homing, weapon `6` Guardian Song player orbit, weapon `3` targeted ray requiring an enemy target, weapon `7` targeted long-lifetime projectile, weapon `8` dual direct bullets sharing the target direction, weapon `4` targetless melee-field case, weapon `10` targetless self-centered direct-field case, weapon `11` direct-fire `DamageJudgeType = None` force case, weapon `11` cardinal force direction cases, weapon `12` owner-forward ray case, weapon `13` owner-forward rect case, weapon `14` targetless owner-forward ray and hit-buff case, weapon `15` targetless freeze field case, weapon `17` targetless stun field case, weapon `18` direct DOT hit-buff case, weapon `20` four-group field timing case, weapon `21` black-hole inward force case, weapon `23` shield self-buff contact-charge case, weapon `24` Iai instant/delayed field case, weapon `25` counter self-buff bullet case, weapon `27` direct friendly buff case, weapon `28` level `1`/`8` direct DOT plus shooter cases, weapon `30` and weapon `33` targeted direct field bullet `60` plus shooter timeline cases, weapon `29` stealth self-buff attribute and buff-level bullet case, weapon `16` basic summon minion firing bullet `22`, weapon `19` fairy minion firing bullet `29`, weapon `22` offensive turret `MinionCount = 2` zero-speed ray bullet `33`, weapon `26` Leo AI-gated minion firing bullet `34`, weapon-level-minion-count, weapon-level-spawn-minion, weapon `32` taunt hit-buff source targeting, shooter `4000` player-side on-destroy event bullet plus source-modified child inheritance, shooter `2002` hostile on-destroy event bullet, active-skill shooter `SpawnPos = 3`, active-skill `13` fan-spread shooter plus non-looping snow field and `LoopFrameInterval = 15` fireball repeat, active-skill `14` frame-1 player-side critical buff snapshot, frame-1 stun field, and frame-90 delayed damage shooter, active-skill `16` delayed stun-field hit-buff level switch through shooters `9000`/`9001`/`9002`, active-skill `114` non-following static damage field shooter `1001`, active-skill `116` owner-forward star-map field plus EXP/coin pickup-gain buffs, active-skill `12` level `1`/`2`/`3` Holy Mend heal/invincible/revive buff parity, active-skill `15` level `1`/`2`/`3` `TargetType = 1` existing-minion buffing plus level-specific minion weapon bullet modifiers, active-skill `117` level `1`/`2`/`3` friendly hit-buff `108` on player and overlapping minion, active-skill `111` level `1`/`2`/`3` summon AI transition shooter chain, active-skill `112` shooter `7000` frame `1`/`3`/`7` timeline plus summon+minion-AI shooter chain including shooter `7003` direction-`0` zero-offset fallback, active-skill `112` level-2/3 same-frame summon groups including shooters `7004`/`7005` loop interval/lifetime and zero-offset bullets `69`/`70`, active-skill `113` / AI `201` first-pass state-22 minion orbit, AIData Boss Cat state-transition-to-shooter variants `2100` through `2104`, AIData Hydra friendly-target fireball shooter with exact event-offset origin and player-side velocity, AIData long Hydra shooter timeline with per-event offsets and velocities, AIState timeline `FireBulletNow`, AIState `IsFireBullet` gating for raw state `FireBulletDatas`, AIState hostile ray damage, AIState timeline `FireAllWeaponNow`, AIState shooter `SpawnPos = 1`, AIState `MoveToRandomPosition`, AIState `Golem_RollAttack` `State_MoveSpeed`, AIState `Samurai_FlashAttack`, AIState `CatBoss_Attack`, AIState BlackCat teleport timeline, AIState entry `buffID`/`buffLevel` active buffs, AIState entry common-state recording, active-skill buff, active-skill summon, and CN `Map_09` through `Map_15` terrain/prefab parity fixture generator/test including enemy/minion boundary clamps; broader CN dump-derived fixtures are still pending |
...
timeline case and same-frame buff/shooter snapshot case: frame `1` applies
player-side buff `9` while creating stun shooter `3000`; level `1`/`2`/`3`
then use frame-`90` shooters `3001`/`3002`/`3003` with bullet `99` attack
`200`/`400`/`600` and the same `UIefx_flash_song` event.
...
The same drop row now also has a deterministic low-rate coin branch test that
spawns and collects CN item `10` through the normal pickup path.
...
- run the route/API/static runtime smoke against the already-running server
  ...
  fields such as `datasets.multiplayConfigData.URL`. The same guard core is
  covered by `npm run test:nfo`, so endpoint regressions can fail before a
  browser or server smoke run.
...
- resolves first-pass shield and counter buffs from `BuffData`: shield/counter
  `Value` is treated as remaining player-damage protection charges, shield
  absorbs enemy contact and hostile bullet damage, and counter absorbs those
  hits while spawning the buff level's `FireBulletDatas` through the normal
  bullet simulation path;
- fires non-counter player-side buff level `FireBulletDatas` once when the
  ...
  emitting bullet `15`; counter buff `6` remains damage-triggered only;
...
- maps `activeSkillData`, exposes the selected character's active skill in the
  ...
  buffs in the local NFO fixture; the current frozen CN snapshot has no real
  same-frame summon-plus-`TargetType = 1` sample to parity-lock. First-pass `AttrChange`
  buffs now affect subsequent pickup and weapon-fire calculations while active,
  including minion movement and direct minion weapon/AI bullet attack, count,
  speed, size, lifetime, cooldown, and crit modifiers for minion-side active
  buffs. Timeline `SpawnMinionData`
  ...
  own `TimeLineEvents`, honor `IsLoopEvent` plus `LoopFrameInterval`, snapshot
  the player's current attack, count, speed, size, lifetime, and crit source
  modifiers when they are created, and emit their direct `fireBulletData`
...
- maps `AIData` into the runtime DTO and attaches `EnemyAITypeID` from level
  ...
  as AI `66` through `70` waiting in state `1` before transitioning to state
  `2` and creating shooters `2100` through `2104`, plus AI `44` state `3`
  firing ray bullet `99` only after its
...
- treats dropped EXP, level-up items, healing, coins, bomb, and magnet pickups
  ...
  parity kill/drop/pickup test for both guaranteed EXP and the low-rate coin
  branch, and CN `DropData` row `20` now has a runtime bomb/magnet/heal
  pickup-effect test. `LevelData.commonDropId` is also covered as the fallback
  drop source when an enemy has no explicit drop ID; boss damage, native
  animation timing, and multiplayer-sharing details remain pending.
...
summon and first-pass minion AI shooter `7003`, plus level-2/3 same-frame
minion AI `205`/`206`/`207` summons with minion AI shooters `7004`/`7005`,
...
boss-style direction `2` samples: shooters `2100` through `2104` use
`BulletHitTargetType = 1` and fire toward the player side from their event
origins. Shooter `2100` / bullet `101` applies event `bulletRotationType = 2`
and keeps `facingAngle` on the rotate-by-core firing angle, while shooters
`2101` through `2104` lock zero event rotation with bullet-data rotate fallback
and the bullet `103` size increase. The fixture also locks shooter
...
parent position when that projectile expires. The parity test also verifies
that parent source modifiers for attack, count, speed, size, and lifetime are
inherited by that event bullet. The same on-destroy path now
...
and stopping their movement. The same frame applies player-side buff `9`
(`灭世之歌暴击buff`, critical rate `+30` for `80` frames), and the parity test
verifies that shooter `3000` and bullet `56` snapshot that critical-rate
modifier. Frame `90` records `UIefx_flash_song` and switches the delayed damage
shooter by level: level `1` uses shooter `3001`
...
The fixture also locks active skill `117` levels `1`/`2`/`3`: frame `1`
creates shooter `11000` in all three levels, whose no-damage friendly bullet
`65` applies invincible buff `108` to the player and an overlapping allied
minion for 30 frames.
It also locks active skill `16` / `KiraKiraDokiDoki` as a delayed stun-field
level-switch case: level `1`/`2`/`3` create shooters `9000`/`9001`/`9002`.
Each shooter emits zero-speed field bullet `59` at the player, keeps
`BulletDamageJudgeDelay = 21`, and only after that delay damages an overlapping
enemy, applies stun buff `18` for 150 frames, and stops that enemy from moving
while stunned. The CN fixture locks `BulletAttack = 200`/`400`/`600` and
`BulletSize = 1000`/`1500`/`2000`.
...
case locks weapon `32` / `皇家警卫团` levels `1` through `8`: `spawnMinionData`
creates minion `10`, uses formation `1` at radius midpoint `4.5`, and overrides
the minion AI from `110` through `117`. Those minion AI states create
player-team shooters `15000` through `15007`, whose timelines emit no-damage
taunt bullet `99` with hit buff `120` and size `500` through `850`; the
simulation records the bullet source on buff `120` and uses that source as the
enemy movement target while the taunt is active. The level-up path for the same
weapon is locked from level `1` to level `2`: after `item-level-up`, the
existing minion is resynced from AI `110` to AI `111`, its next AI action
creates shooter `15001`, and shooter `15001` emits the larger level-2 taunt
bullet `99` with size `550` instead of level-1 size `500`. A max-level runtime
case also verifies level `8` creating AI `117`, shooter `15007`, and taunt size
`850`. The AI parity case locks
AI `66` through `70` starting at
`FirstStateID = 1`, following the state `1` -> state `2` transition after 30
frames, and creating `BulletShooterID` values `2100` through `2104`; the
simulation verifies that these hostile shooter paths emit player-damaging
bullets `101` through `105`. The AI shooter
...
levels `1`/`2`/`3`: each frame-1 timeline event applies a player-only
`AttrChange` self buff lasting 240 frames. Level `1` uses buff `10` with attack
`+15`, defense `+2`, bullet size `+30`, critical rate `+15`, and critical
damage `+15`; level `2` uses buff `14` with attack `+30`, bullet size `+45`,
critical rate/damage `+30`, and bullet count `+1`; level `3` uses buff `15`
with attack `+50`, bullet size `+60`, critical rate/damage `+50`, and bullet
count `+2`. The parity test verifies the buffs remain player-only and that
subsequent CN weapon `1` / bullet `11` fire inherits the attack, bullet-size,
bullet-count, and critical source modifiers.
It also locks skill `12` / `Holy Mend` levels `1`/`2`/`3`: each frame-1
timeline event applies the same `TargetType = 1` revive buff `106`, invincible
buff `104`, and heal-percent buff `105`. The current simulation verifies the
heal-percent, invincible, and first-pass single-player revive effects for all
three levels while keeping native ally revive targeting as pending behavior.
The same active-skill buff fixture set now locks skill `15` / `Fairy Guard`
levels `1`/`2`/`3`: frame `1` carries `TargetType = 1` stat buffs
`11`/`16`/`17`, respectively, plus shared invincible buff `13`. The runtime
parity test verifies those persistent buffs apply to both the player and a
currently existing player-side minion without applying to enemies. It also
verifies CN weapon `16` / bullet `22` uses the level-specific minion-side attack
...
Skill `112` level `2`/`3` additionally locks the same-frame summon events for
AI types `205`/`206` and `205`/`206`/`207`; the middle event keeps the base
radius-`250` placement and minion AI shooter `7004`, which emits bullet `69`,
while level `3`'s third event uses
...
ring placement and retains the assigned weapon metadata. The level-`1` parity
case now advances that summoned minion into its assigned weapon fire: weapon
`28` level `8` emits ten DOT bullet `5` hits with buff `4`, creates shooter
`9`, and shooter `9` emits the first two bullet `24` fan bullets at speed
`1300`.
...
- Current minion handling maps CN `WeaponData.weaponType`, `minionID`,
  ...
  The CN guard path is locked across levels `1` through `8`: AI `110` through
  `117`, shooters `15000` through `15007`, and no-damage taunt bullet `99`
  sizes `500` through `850`. Buff `120` redirects affected enemy movement plus
  hostile direct-fire and shooter friendly-target aim to the bullet source in
  the first-pass simulation. Deeper native taunt threat priority, retarget
...
- Current weapon self-buff handling maps frozen CN runtime/typetree
  ...
  contact damage and hostile bullet damage, consuming one `Value` charge per
  protected hit; CN weapon `23` / buff `5` level `1` locks the two-charge shield
  case from frozen data, including a hostile CN AI `44` bullet `99` hit. Type `6`
  counter buffs now absorb enemy contact damage and hostile bullet damage,
  consume one `Value` charge,
  and spawn the active buff level's `FireBulletDatas` through the same bullet
  creation and collision path as weapon fire.
  CN weapon `25` / buff `6` level `1` locks `Value = 1` plus zero-speed bullet
  `27` with `BulletAttack = 100`, `BulletSize = 300`, `BulletLifeTime = 30`,
  and `BulletDamageJudgeDelay = 10`, including a hostile CN AI `44` bullet `99`
  trigger. These fields were observed in the frozen CN runtime resources, not
  yet as named members in the available `dump.cs` class text. CN weapon `29`
  now locks a first-pass
...
- Current bullet component rotate handling follows the CN dump enum:
  ...
  `facingAngle = angle` rotate-by-core behavior. The same Boss Cat fixture now
  locks zero event-rotation variants `2101` through `2104`, including bullet
  `102` falling back to `BulletData` rotate type `1` and bullets `103` through
  `105` falling back to rotate type `0`. Because the event rotation
  and `BulletData` rotation are both `2`, the `2100` fixture does not disambiguate
...
- Current hit-target handling maps CN `BulletHitTargetType`. Type `0` remains
  ...
  CN active skill `117` locks the active-shooter allied-minion overlap:
  shooter `11000` bullet `65` applies invincible buff `108` to both the player
  and an overlapping allied minion. CN weapon `30` now locks the first
  weapon-shooter allied-minion friendly hit-buff overlap: shooter `301` bullet
  `99` applies buff `109` to both the player and an overlapping allied minion.
  CN weapon `27` locks the direct
  friendly-target path: level `1` emits ten moving bullet `32` projectiles with
  `HitBuffID = 7`, and the resulting player-side buff carries defense `+1` and
  speed `+50` for 60 frames on both the player and an overlapping allied
  minion. Broader native ally selection, ally priority, and non-overlap
...
- Current special-buff handling covers the self-buff player-damage protection
  subset observed on CN `浮游光盾` and `弹反`: `Shield` and `Counter` use `Value`
  as remaining protection charges for enemy contact and hostile bullet hits,
  ...
  player damage cooldown is applied. CN active skill `117` levels `1`/`2`/`3`
  / shooter `11000` now lock type `9` invincible as a player-plus-overlapping
  minion friendly hit-buff path, not only as a direct `AddBuffDatas` path. CN
  active skill `15` now locks
  `TargetType = 1`
  active-skill `AddBuffDatas` as player-plus-existing-minion targeting for
  persistent buffs, and direct minion weapon/AI bullet sources now read their
  own active-buff projectile modifiers instead of implicitly inheriting
  self-only player buffs. Local NFO coverage also locks same-frame spawned
  minions as valid `TargetType = 1` buff targets when a frame contains both
  summon and player-side buff events. It also locks same-frame active-skill
  player buffs as source modifier snapshots for newly created active-skill
  bullet shooters, including later loop shots after the short-lived buff has
  expired. Minion heal/revive HP semantics are still absent because the
  first-pass minion model has no HP/death lifecycle. Native ally selection,
  teammate-alive invalidation, exact shooter modifier timing beyond this local
  same-frame fixture, and exact revive animation timing remain pending.
...
- Current active skill handling maps CN `activeSkillData`, level charge frames,
  ...
  CN skill `11` / `Demon God Descends` locks self-targeting buffs `10`/`14`/`15`
  for level `1`/`2`/`3`, including attack, defense, bullet size, bullet count,
  critical rate, and critical damage source modifiers.
  CN skill `12` / `Holy Mend` locks level `1`/`2`/`3` frame-1 player-side
  revive, invincible, and heal-percent buffs `106`/`104`/`105`; all three
  levels share that CN buff set.
  CN skill `15` / `Fairy Guard` locks `TargetType = 1` buffs for levels
  `1`/`2`/`3`: stat buffs `11`/`16`/`17` plus shared invincible buff `13`, with
  existing player-side minion weapon fire inheriting the level-specific attack
  and bullet-size modifiers.
  ...
  `117` verifies level `1`/`2`/`3` shooter `11000` applying invincible buff
  `108` through bullet `65` to the player and an overlapping minion, while CN
  skill `14` verifies shooter `3000`
  applying stun buff `3`
  and movement disable through field bullet `56`, and CN skill `16` verifies
  shooters `9000`/`9001`/`9002` applying delayed stun buff `18` plus movement
  disable through field bullet `59`.
  Shooter instances use `LifeTime`, `SpawnPos`, `SpawnPosOffsetX/Y`,
  `TimeLineEvents`, direct `fireBulletData`, `eventFireBulletDatas`,
  `IsLoopEvent`, and `LoopFrameInterval` at a first-pass level. CN shooters
  `6000`/`6001`/`6002`, tied to active skill `110` levels `1`/`2`/`3`, lock
  ...
  current no-target +X first-pass fallback. Its level-2/3 paths also lock same-frame `SpawnMinionData` events,
  including the middle AI `206` minion at radius `250` with shooter `7004` /
  bullet `69` in both levels, plus the level-3 offset AI `207` minion at `(250, 250) + radius 1` with
  ...
  `WeaponID = 28` level-`8` formation-2 summons plus level-`1` assigned weapon
  fire, `Galaxy Star` level `1`/`2`/`3`
...
- Current enemy AI handling maps CN `AIData`, including `FirstStateID`,
  ...
  ordinary firing AI, boss shooter links such as AI `66` through `70`
  state `1` -> state `2` -> shooters `2100` through `2104`, AI `32`
  state `4` -> shooter
