...
Light` as an active-skill entry case for shooter `6000`, including
`UIefx_flash_starlight`, owner-following shooter creation, six-bullet radial
fire, and `LoopFrameInterval = 10`.
...
Active skill `110` / `Ultimate Heart Light` level `1` now locks the active-skill
entry for shooter `6000`: frame `1` records `UIefx_flash_starlight`, creates an
owner-following shooter with `LifeTime = 55`, and emits six radial bullet `28`
projectiles that loop every 10 frames.
...
- Current active skill handling maps CN `activeSkillData`, level charge frames,
  ...
  `6000`, now tied to active skill `110`, locks the first snapshot-derived
  loop/lifetime case: frame-1 six-bullet radial fire repeats at frame `11`,
  the shooter expires when age reaches `LifeTime = 55`, and the skill records
  `UIefx_flash_starlight` as a serializable full-screen effect event. CN active skill `13` / shooter `13000`
