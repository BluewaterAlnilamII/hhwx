...
- maps `activeSkillData`, exposes the selected character's active skill in the
  ...
  first-pass active skill timeline. Timeline `AddBuffDatas` currently apply
  buff levels to player-side active buff state, and first-pass `AttrChange`
...
- Current special-buff handling covers the self-buff contact subset observed on
  ...
  direct `AddBuffDatas` path. Native ally selection, teammate-alive
  invalidation, and exact revive animation timing remain pending.
...
- Current active skill handling maps CN `activeSkillData`, level charge frames,
  ...
  targeting beyond the current player-plus-overlapping-minion first pass remain
