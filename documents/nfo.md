...
`65` applies invincible buff `108` to the player side for 30 frames.
...
- Current hit-target handling maps CN `BulletHitTargetType`. Type `0` remains
  ...
  CN weapon `30` now locks the first weapon-shooter allied-minion friendly
  hit-buff overlap: shooter `301` bullet `99` applies buff `109` to both the
  player and an overlapping allied minion. CN weapon `27` locks the direct
...
- Current special-buff handling covers the self-buff player-damage protection
  ...
  / shooter `11000` now lock type `9` invincible as a friendly hit-buff path,
  not only as a direct `AddBuffDatas` path. CN active skill `15` now locks
  `TargetType = 1`
...
- Current active skill handling maps CN `activeSkillData`, level charge frames,
  ...
  `108` through bullet `65`, while CN skill `14` verifies shooter `3000`
