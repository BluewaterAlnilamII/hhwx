...
- maps `activeSkillData`, exposes the selected character's active skill in the
  ...
  buffs. Same-frame newly spawned minion buff targeting remains pending.
...
- Current special-buff handling covers the self-buff contact subset observed on
  ...
  self-only player buffs. Minion heal/revive HP semantics are still absent
  because the first-pass minion model has no HP/death lifecycle. Native ally
  selection, teammate-alive invalidation, same-frame summon targeting, exact
  shooter modifier inheritance timing, and exact revive animation timing remain
  pending.
