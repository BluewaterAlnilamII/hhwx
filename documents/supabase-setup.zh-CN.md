...
如果既有项目已经执行过旧版 `auth_schema.sql`，还需要执行 `documents/comment-likes-notifications-schema.sql`，补齐评论点赞、提醒记录和评论点赞计数。随后执行 `supabase/migrations/20260630053053_comment_reactions.sql`，添加 emoji reactions 并回填既有点赞，再执行 `supabase/migrations/20260630055412_retarget_legacy_comment_reaction_kokoro_yay.sql`，把迁移后的旧点赞改为 `KokoroYay`。
