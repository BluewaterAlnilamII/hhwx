...
Shared controls use search, server, band, attribute, rarity, character, card type and sort rows in that order. Scenario-specific servers and power sorting remain available where applicable. Light surfaces use the panel theme token (`#FFFEFA`), and selected filter text remains neutral. Compact thumbnail grids, virtualization, card-art toggles and confirmation actions retain their existing behavior. Dark styling is unchanged.

The shared toolbar uses a magnifier submit button, `Clear`, and immediate `?` help. Selected filter buttons use a pale information-blue fill with neutral text. Result counts retain information-blue text and borders on the panel background, with tabular digits reserving at least four character widths. The existing theme tokens and shared components own these styles.
