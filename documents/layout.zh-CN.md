...
- `bandori/`：按领域整理的 Bandori 模块。`bandori/team-builder/shared/` 包含 single/medley 组队搜索共享计算 helper，`bandori/team-builder/single/` 包含单曲 exact 搜索编排，并由旧的 `bandori-team-search.ts` facade 对外兼容导出。
