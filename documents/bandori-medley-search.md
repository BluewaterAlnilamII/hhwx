...
Current checkpoint (2026-09-01): after the accepted fourteen-scene small-roster rerun, clean commit `8c758bf` ran all 48 authorized scenes across twelve historically four-scene-exact profiles over 1,000 cards. All 48 reached or exceeded their same-input primary references, so the score-regression objective passed. Forty-five completed exact (42 equal, 3 higher). The other three—1229/event244, 1513/event244 and 1703/event260—reached the primary scores but timed out at 300 seconds; only their optimality proofs remain incomplete. Scoring and search were unchanged.

Native search totaled 3,606.484 seconds; sampled process peak was 40.36 MiB and budgeted search-storage peak 32.16 MiB. A provenance audit rejected five extra comparisons because four used a different profile payload and one used different event parameters and experimental code. The only remaining issue in this batch is the three proof timeouts. [The fixture procedure](bandori-medley-fixtures.md) indexes the complete private results and preserves the final five high-pressure profiles, which were not run. No budget increase, historical-team replay, algorithm change or browser/WASM acceptance is implied.

Accepted joint-work cutoff (2026-09-01): in the three timeout cases, 6.7%–20.6% of joint passes already had a whole bound below the incumbent. Stopping those passes before backward and per-card destination work increased 60-second node progress by 44.6%–55.8% and bound evaluations by 46.9%–56.0%, with the same or higher scores. The 961-card exact regression retained the identical result and completed in 6.06 seconds instead of 8.47 seconds. This changes no surviving-node bound or branch.

Rejected root-only experiment (2026-09-01): forcing every Cartesian choice of the three existing linear weights for the three songs and intersecting all 27 safe bounds changed neither the whole bound nor any allowed card-destination bound in the three timeout cases. The automatically selected weight was already the best in every song, so this adds 27 joint solves without pruning value and will not enter DFS.

Rejected root-context experiment (2026-09-01): separating same/mixed band and attribute cases one team and one axis at a time was safe but reduced the three root bounds by only 3,369–5,099 points (0.033%–0.057%) after 42 extra joint solves taking 0.48–0.62 seconds per configuration. This removed only 0.68%–1.09% of the remaining proof gaps and cannot prune those roots. Existing fixed-member checks already restrict a team to mixed contexts once selected cards make sameness impossible, so the root expansion will not enter DFS.

Rejected branch-order experiment (2026-09-01): choosing cards with fewer remaining destinations first increased 60-second node visits by 46%–61% in the three timeout cases, but slowed the 961-card exact proof from 6.06 to 18.43 seconds. Keeping the maximizing-allocation preference first and using destination count only within it still took 9.06 seconds. Both retained the identical exact winner, but neither ordering will replace the current proposal-and-conditional-gap order.

Rejected segmented root bound (2026-09-01): eight parameter intervals with two safe product planes reduced the three root bounds by 30,296–92,785 points and raised immediate destination cuts from 3/137/1,690 to 1,487/2,113/3,358. Sixteen intervals gave no further reduction in two cases and only 4,416 points in the third. Despite the stronger cuts, applying the eight-bin result reduced 60-second node progress by 5.3%–25.5%; applying only its whole bound also slowed the 961-card exact run from 6.06 to 11.04 seconds. The root prototype is removed; any future segmented bound must justify a lower-frequency DFS placement rather than charging every configuration.

Rejected destination saturation (2026-09-01): stopping a card-destination scan once its partial upper reached the incumbent skipped 56.36 million remaining pattern visits across 2.81 million destinations, but replacing their reusable conditional values with the whole bound destroyed later pruning and ordering value. The 961-card case completed only 4 of 108 configurations in 60 seconds instead of proving exact in 6.06 seconds. The prototype was removed without running the timeout cases.

Rejected one-level joint deferral (2026-09-01): reusing the safe parent table for one child level deferred 1,883 rebuilds in the 961-card case, but widened its proof tree from 3,412 to 7,641 nodes and raised bound evaluations from 194,214 to 357,227. The identical exact winner then took 9.90 instead of 6.06 seconds, while total joint calls still increased. The prototype was removed without running the timeout cases.
