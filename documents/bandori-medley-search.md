...
Two implementation changes are retained. First, 6.7%–20.6% of joint passes in the three timeout cases already lost after the forward whole bound. Stopping there raised 60-second node progress by 44.6%–55.8%; the identical 961-card exact result took 6.06 instead of 8.47 seconds. Second, the joint DP has at most 1,000 states, so transition endpoints now use exact 16-bit indexes instead of machine-word indexes. The instrumented 961-card exact run kept all non-time counters and improved from 7.80 to 7.53 seconds. In 30-second timeout runs, node progress changed by -0.9%, +5.8% and +3.6%, while sampled process peaks fell by 7.8%–11.6%. Neither change alters surviving bounds, floating-point order or branches.
