...
Three implementation changes are retained. First, 6.7%–20.6% of joint passes in the three timeout cases already lost after the forward whole bound. Stopping there raised 60-second node progress by 44.6%–55.8%; the identical 961-card exact result took 6.06 instead of 8.47 seconds. Second, the joint DP has at most 1,000 states, so transition endpoints now use exact 16-bit indexes instead of machine-word indexes. The instrumented 961-card exact run kept all non-time counters and improved from 7.80 to 7.53 seconds; 30-second timeout progress changed by -0.9%, +5.8% and +3.6%, while sampled process peaks fell by 7.8%–11.6%. Third, forward, backward and destination passes now skip impossible table entries before arithmetic. The same 961 result improved from 7.32 to 6.91 seconds; timeout progress changed by +2.0%, +6.4% and -0.9%. These changes alter no surviving bound, floating-point order or branch.

Rejected prototypes remain deleted; Git retains their implementation history.

| Prototype | Decisive result |
| --- | --- |
...
| Fewer-destinations-first branching | 961 exact slowed from 6.06 to 18.43 seconds; the conservative variant took 9.06 seconds. |
