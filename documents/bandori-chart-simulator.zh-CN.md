...
“显示 Perfect 判定区间”和“显示 Great 判定区间”默认关闭。开启 Great 会同时开启并锁定 Perfect；关闭 Great 后，Perfect 会保持开启，直到用户自行关闭。它们只是一维时间轴可视化，不增加触摸输入，也不改变模拟器的 AutoPerfect 行为。设一个原生帧为 `F = float32(1 / 60)` 秒，标准点按在 `abs(delta) <= 2.5F` 时属于 Perfect；两侧 Great-only 区间为 `2.5F < abs(delta) < 5.5F`。
...
Slide 使用可跨歌曲保存的手动判定帧补正 `c`：范围为 `+0.0F...+1.0F`，步长 `0.1F`，默认 `+0.5F`。谱面起点和普通终点的 Fast 侧 Perfect 宽度为 `(2+c)F`，谱面中间计分点仅为 `cF`；起点和普通终点在其外侧另有三帧 Great-only，即从 `(5+c)F` 到 `(2+c)F`，中间点不显示 Great。Flick 与 Directional Slide 尾端不能提前完成，因此 Fast 侧长度为 0，也没有 Fast Great。该诊断设置不会再根据 Note Speed、设备场地几何或 float32 位置表累加自动推断相位。不变调慢放时，只有这些 Fast 帧偏移随画面接近时钟缩放。
...
每条色带只横跨所属 Note 自己连续覆盖的按键。普通 Note 与 Long 显示 Fast、Slow 双侧；Slide 在接近判定线时也会显示名义 Slow Perfect 范围，但整条色带仍会在所属 Note 于 `T` 被 AutoPerfect 触发时一起消失，包括恰好位于 `T` 的渲染帧。高对比度边框会跟随每一段实际归属范围，使 Note 之间的分割边界保持可见。多个仍存活的标准按下 Note 共用同一按键时，以原始目标时间的中点分割归属。只有原生可触发区间包含该时间切片的候选才会参与抢判定：标准按下使用 `abs(delta) < 7.5F`，Slide 头部使用其 Fast 侧直到 Bad 的完整区间，以及既有 Slow 超时／中点截止区间；Good 与 Bad 只作为内部可触发边界，不增加诊断色带或播放行为。前一个 Note 被 AutoPerfect 并清除后，后一个 Note 会在下一渲染帧恢复其名义区间中仍位于未来的部分。Long 释放不参与这项分割，因此重叠的尾端色带保持彼此独立。

尚未绑定的 Slide 头部按当前投影位置到判定线的距离参与竞争；它与普通候选完全等距时普通候选优先，并固定按原生 `JudgementAdjustValueB = 0` 计算。头部绑定后，同一 Slide 的谱面中间节点和尾端保持独立，不再重新选取附近 Note；较早头部触发并清除后，相邻 Note 仍会在下一渲染帧恢复尚未过去的名义范围。该可视化把触摸位置折叠为一维：每条 Slide 色带始终固定在所属计分节点自己的覆盖按键上，表示“该时间切片到达判定线时在这些按键操作”，不沿连接带插值，也不包含原生临轨命中圆。隐藏几何样本本身不会获得色带。镜像只映射所属按键，不另设隐藏规则；色带也不受 Sudden 遮罩影响。

谱面模拟器只用 Slide 的适用期限与中点边界裁剪诊断用 Slow Perfect 色带，不实际执行 Miss 状态，也不模拟触摸或 finger 状态、尾端重新接入、滑动位移与方向、抬手失败、临轨触摸容差或其它二维输入选择；这些原生分析结果不改变播放逻辑。判定公式仍保留开闭区间语义，但零宽度的单个边界点不会另画一个像素色带。同一按键、同一时刻的重复标准按下 Note 属于无效谱面输入，不增加模拟器专用的同刻优先逻辑。
