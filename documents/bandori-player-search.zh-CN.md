...
玩家查询接受 1–20 位 ASCII 十进制数字，最大值为 uint64 上限 `18446744073709551615`。
输入框保留前导零，服务端在查询前去掉前导零；UID 全程保持字符串，避免精度丢失。
账号绑定和档案同步继续使用各自原有的 UID 校验。

浏览器调用 `GET /api/bandori/player/{server}/{uid}`，成功响应为
`{ success, data: { server, uid, fetchedAt, profile } }`。私有后端以 `gameUid` 表示 UID。
支持 JP、EN、TW、CN 四服；账号绑定验证通过同一实时查询读取公开的 `profile.introduction`。

每次查询均同步抓取游戏资料，最多等待该服账号通道 5 秒。后端不保留玩家资料缓存、
后台刷新或缓存回退。页面再次查询时刷新结果，不自动轮询。暂时性刷新失败时，
浏览器保留之前的结果；明确不存在或响应异常时清除。`fetchedAt` 只用于展示，
不因资料年龄或时钟偏差判定响应无效。

删除 `mode` 参数；请求中出现该参数（包括空值）时返回 `400 INVALID_BANDORI_PLAYER_MODE`。
成功响应不再包含 `mode`、`cache`、`refreshError`，同时删除 `BANDORI_PLAYER_CACHE_MISS`。
后端与 Web 应在同一发布窗口更新，后端先发布；旧调用方须移除 mode，不提供兼容期。
Web 拒收旧成功字段，避免账号绑定验证误用旧后端的缓存结果。

API 成功和失败响应均为 `no-store`。私有后端地址及 token 仅保留在服务端。
请求超时为 30 秒，不允许重定向，响应正文上限为 1 MiB，并核对返回的玩家身份。
错误响应不包含后端正文。明确的 `BANDORI_PLAYER_NOT_FOUND` 返回 `404`，包括已核实的
游戏“目标玩家不存在”响应；其他游戏 `400` 不直接视为玩家不存在。错误码缺失、未知或与
HTTP 状态不符时，返回 `502 TRACKER_SERVICE_INVALID_RESPONSE`。繁忙、维护、bot 会话不可用
使用不同错误码和 `503`；超时为 `504`，解密、解码或其他响应异常为 `502`。
其他上游故障返回 `502 TRACKER_SERVICE_FAILED`。内部认证失败属于服务故障，
不误报成访客权限问题。公开 API 和本地开发代理仅转发允许列表内的错误码。
...
限制为每 IP 每 10 秒 2 次，超限阻止 10 秒。同一 IP 的四服和不同 UID 查询在各个
Cloudflare 数据中心内共用计数。账号绑定、档案同步和评论接口不纳入。
规则使用统一 JSON 错误格式返回 `429 BANDORI_PLAYER_RATE_LIMITED`。
页面显示“同一IP操作过于频繁，请稍后重试”，并保留已有结果。本地开发代理保留此错误；
...
路由会转发到固定的 `https://hhwx.org/api/bandori/player/{server}/{uid}`，不携带 mode 参数，
校验公开响应结构和玩家身份，再执行本地隐私过滤，沿用超时和正文上限；目标须使用同一实时查询合同。
