...
最新的恢复后 40-case 证据是 `temp/bandori-team-builder/focus-medley-cases-2026-06-04T14-14-48-198Z.json`。该轮证明 `37/40` exact，没有 failed 或 timeout；bounded gap 总量从 `1534986` 降到 `1012798`；P95 为 `219289ms`，max 为 `293227ms`，采样到的 peak working set 为 `4201.2 MiB`。峰值行是 `P02:260`；进程 working set 是诊断采样，不是引擎内部硬内存上限。
...
- 截至 2026-06-04 恢复后矩阵，剩余 bounded row 是：
  - `P02:260`：pair-upper bounded，gap `416575`，也是本轮最高 working set 采样，约 `4201.2 MiB`。
  - `P04:260`：小 gap bounded，gap `41696`，elapsed `293227ms`。
  - `P10:244`：即使 guarded extension 到 `600000`，仍是 candidate-fill soft-limit bounded，gap `554527`。
- `P08:260`、`P08:323` 和 `P10:260` 在固定矩阵中已不再是当前回退点。仍应保留在 `focus-6-300` 中，避免 third-shortlist 和 guarded-fill 相关修改静默回退。
- 最大的剩余证明机会是为 `P02:260` 提供更紧的 pair/frontier upper closure、为 `P04:260` 做更便宜的小 gap proof sprint，以及为 `P10:244` 设计更有选择性的 candidate-fill 策略。
