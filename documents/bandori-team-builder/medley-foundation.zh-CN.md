...
单曲复用这里的基础，另加加权站位与活动规则，见[单曲组队计算器](single-song-algorithm.zh-CN.md)。下文三曲组曲规则保持不变。
...
计算器 Worker 在单曲或组曲搜索前，用计算器明确设置的 `excludedCardIds` 偏好覆盖档案中的排除标记，避免导入的排除状态悄悄移除候选卡。已保存的档案和编解码器仍保留这些标记以维持兼容；底层搜索适配器仍遵守收到的有效输入中的排除标记。

临时卡牌以卡牌主数据 ID 标识。再次选择相同 ID 时会编辑已有临时卡，而不是增加一张重复卡。首次选中时，界面会填入该卡允许的最高可选参数，用户随后可以修改。计算时，临时卡覆盖档案中相同主数据 ID 的卡牌。计算器的排除设置只作用于该档案卡，因此不会排除覆盖它的临时卡。临时卡只属于当前会话输入：既不写入已保存的游戏档案，也不写入本地卡牌偏好；刷新页面或切换档案都会将其清除。
...
3. 有效搜索输入中标记为排除的卡牌不是合法候选。在计算器中，这个标记仅来自明确设置的排除偏好，不排除覆盖档案卡的临时卡。
...
4. 对每项参数，先将上一步所得参数乘以潜能解放倍率并向下取整；再独立将同一参数乘以“收集任务加成 + 培养任务加成”倍率并向下取整，最后相加得到两项整数加成之和。
5. 把所得角色加成加入卡牌，得到 `characterParameter`。
6. 再以此计算匹配的区域道具和活动贡献。

例如，某项参数为 1,530，潜能解放为 3%，收集与培养任务合计为 9%，则：

```text
角色加成 = floor(1,530 * 0.03) + floor(1,530 * 0.09) = 45 + 137 = 182
角色参数 = 1,530 + 182 = 1,712
```

潜能解放和任务加成分别向下取整；收集与培养任务倍率先合计，再进行任务加成的取整。即使 P/T/V 三项恰好相同，也不会切换到另一种规则。

当前计分规则版本为 `hhwx-medley-bestdori-v5`。它保留 `v4` 引入的参数分别取整，并修正效果字段经过排序后的连续 PERFECT 技能解析。原 `v3` 在取整前合并潜能与任务倍率，可能高估卡牌参数及后续区域道具／活动加成。旧规范输入需要从原始档案和主数据重新生成；仅改版本标签不能修正预先计算的值。存档格式保持不变。TypeScript 转换器与重新生成的 WASM 包需一同部署，确保两者的规则版本一致。
...
全队统一加成值可能取决于五名成员，因此技能要在完整队伍确定后解析。只有一项已识别计分效果时，它就是主要效果。若同时有 `score_continued_note_judge` 和普通 `score`，则无论序列化字段顺序如何，持续 PERFECT 都是主要效果，`score` 是降档效果。若同时有 `score_over_life` 和 `score_under_life`，现有无生命状态模型固定选择 `score_over_life`。其他计分效果组合与未知的 `score*` 效果直接报错。每项已识别效果都必须有可解析、非负的区服数值；显式零仍然有效。合法的统一加成值在乐团或属性条件与完整队伍匹配时，只覆盖主要效果的百分比。规范行为包括：
...
普通计分技能还可以带 `isRateUpWithPerfect`；每个 PERFECT 的增量和上限由计分规则固定，不由调用方输入。模型没有生命输入和生命状态；含生命名称的计分效果作为普通计分效果使用，不检查生命阈值。
...
已识别的计分实体会先校验再转换：每个保留节点必须是对象，且 `beat` 为有限数值；Long 和 Slide 必须提供至少含两个端点的数组；每条 BPM 的 `beat` 必须有限，`bpm` 必须有限且大于零。允许有限数字和非空数字字符串，拒绝缺失值、空白字符串、`null`、布尔值及非有限值。Long 内部数据、带 `hidden` 的 Slide 中间节点，以及未知／非计分实体仍会忽略；这里不对未使用字段实施完整谱面 schema 校验。

必需数据损坏时返回 `INVALID_CHART`，并保留原始来源路径，例如 `sourceInput.songs[0].chart[7].beat` 或 `chart[8].connections[1].beat`。固定队评估与组队搜索共用这一转换器。即使六个合法触发仍然存在，损坏的普通音符或 BPM 也必须在计分／搜索前拒绝；静默删除它会让程序证明另一张谱面的最优解。此校验不改变合法规范谱面的输出。
...
组曲 v4 明确保留固定触发窗口和均匀顺序。单曲的加权顺序模型及技能结束后 0.75 秒排队不会进入这条路径。共同计分规则和未来时间机制迁移时需要保留的模式差异，见[对齐审计](single-song-algorithm.zh-CN.md#单曲组曲对齐审计)。
...
对 JP 原生客户端 10.1.3、主数据版本 `20260805110509` 和技能效果 SHA-256 `d98e76c0198a6a714be1d38e4696a044242c8384b905426a311c8c2b0961aebc` 的审计还发现了本计算器模型之外的行为：有偏随机技能顺序抽样、单精度计分、combo 主数据表、生命条件触发，以及逐帧处理技能窗口冲突。组曲不模拟这些行为；迁移后的单曲计算器单独考虑有偏技能顺序。若改变这项边界，需要建立新的计分规则版本和对应参考用例。
...
| 单曲组队计算器 | 迁移后的 Rust 搜索保留综合力小数参与计分，显示值与组曲共用 float32 转换后截断函数。 |
| 单曲支援乐队 | 对原始支援合计值向下取整后显示，对齐 Bestdori 的 `Math.floor(entry.supportBP)`。搜索与活动点数计算仍保留支援小数。这保留[单曲组队计算器](single-song-algorithm.zh-CN.md#计分范围)的支援计分契约，不模拟原生支援参数。 |
