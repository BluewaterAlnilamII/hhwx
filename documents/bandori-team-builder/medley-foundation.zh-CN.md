...
必需数据损坏时返回 `INVALID_CHART`，并保留原始来源路径，例如 `sourceInput.songs[0].chart[7].beat` 或 `chart[8].connections[1].beat`。固定队评估与组队搜索共用这一转换器。即使六个合法触发仍然存在，损坏的普通音符或 BPM 也必须在计分／搜索前拒绝；静默删除它会让程序证明另一张谱面的最优解。这次校验收紧不改变合法规范输入的输出，也不改变 `hhwx-medley-bestdori-v3`。
