...
此参数规则的版本为 `hhwx-medley-bestdori-v4`。原 `v3` 在取整前合并潜能与任务倍率，可能高估卡牌参数及后续区域道具／活动加成。旧规范输入需要从原始档案和主数据重新生成；仅改版本标签不能修正其中预先计算的参数。存档格式保持不变。TypeScript 转换器与重新生成的 WASM 包需一同部署，确保两者的规则版本一致。
