...
组曲搜索是独立优化问题，见 `medley-algorithm-notes.md`。
...
5. 对五个不同角色做分支定界 DFS。
6. 对通过上界检查的完整队伍，解析真实技能上下文并精确评分。
7. 按目标值排序，再用分数或综合力同分规则、队长技能强度和稳定卡牌 ID 排序。
...
第一层上界便宜，递归中频繁调用。第二层上界只在接近当前阈值时尝试，用小型 Pareto/DP 联合估计剩余综合力、技能贡献和 PT 加成，避免把不同卡上的独立最大值错误组合。如果更紧上界无法证明安全或成本过高，搜索回退第一层上界。
...
- `src/lib/bandori/team-builder/shared/`：旧兼容门面。新的内部代码应直接从 `core` 或 `single` 导入。
- `src/lib/bandori-team-search.ts`：公开兼容入口，重新导出单曲搜索 API。

`core` 层不能导入 `single`、`medley` 或 `shared`。
