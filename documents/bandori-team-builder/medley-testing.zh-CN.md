...
`npm run test:team-builder` 对实际提交的单曲 WASM 绑定运行独立加权顺序／站位及逐音符枚举对照。公开样本覆盖：

- 页面允许的全部 13 种活动／演出组合、两个 API 目标和公式版本 0/1/2，共 78 个用例，包括 `festival + versus`。先验证原始加成到综合力、Pt 加成和支援力的规范化，再与完整队伍／站位枚举比较；沿用的 TypeScript Pt 函数作为 Rust 活动公式的独立参考。
- 协力期望与最高分站位、可达最高分、输出发动顺序、并列概率和房间分，对五种安可来源分别枚举全部 120 种房间顺序；同技能还要求概率为 120/120。
- 全部 Live Boost、挑战 CP、名次和团队演出胜负的展示选项，使用独立公式参考并覆盖分段阈值附近的分数。
- 完整五张任务支援、同角色不同卡、排除卡、可满足的综合力门槛，以及为保留支援力而改变最优主队的用例。
- 区服回退、外部技能条件各自独立、同时刻音符、技能结束端点、中止／错误与无解。

JavaScript 逐音符参考使用无区域道具配置；`single.rs` 测试覆盖多套道具，并以完整枚举验证搜索和每个部分分支的上界。这些小样本不证明完整档案性能或当前所有区服客户端的随机规则。独立模型／版本契约见[单曲组队计算器](single-song-algorithm.zh-CN.md)。
