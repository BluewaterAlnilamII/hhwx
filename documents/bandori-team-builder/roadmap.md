...
- raw anchor/frontier hard-row probe is now available behind `HHWX_LOW_MEMORY_RAW_ANCHOR_FRONTIER_PROBE=1`; it is no-op and records only a diagnostic `residualUpperBound`, never a proof result;
- retained P02 smoke `low-memory-polish-hhwx-2026-06-17T20-55-33-203Z.json` completed with unchanged result fields: bounded gap `382812`, score/average `9376984`, max score `9412868`, peak `2943 MiB`; the raw probe built the `747802`-candidate pool in `306ms`, built containing bitsets in `76ms`, processed `2841` anchors in `482ms`, and reported `0` mismatch / `0` score-order violations;
- retained six-row pressure gate `low-memory-polish-hhwx-2026-06-17T20-58-54-247Z.json` with the raw anchor/frontier probe preserved the clean proof shape: `4 exact / 2 bounded`, bounded gap `582812`, `0 failed / 0 timedOut / 0 memoryLimited`, peak `2952 MiB`, and all six rows retained `averageScore` and `maxScore`;
- the probe triggered only on `P02:260` in that focused gate; it produced `residualUpperBound = 9583920`, residual gap `206936`, `wouldClose = false`, source `generated-pair`, raw retained `28.53 MiB`, containing bitsets `10.97 MiB`, elapsed `419ms`;
- conclusion from the hard-row probe: raw-index scans make large-pool diagnostics cheap enough, but the current cheap-upper shape remains too loose to close P02. The next proof-relevant slice should tighten the `generated-pair` source, likely with a conflict-aware raw split / pricing-style pair upper, rather than rerunning the same loose anchor scan earlier or with a larger budget;
...
6. Promote raw candidate pool from profile to opt-in resident infrastructure:
   ...
   - the raw anchor/frontier probe proves large-pool raw scans are affordable on P02, but also shows the current generated-pair upper is too loose; prioritize conflict-aware raw pair-upper tightening before spending more time on broad anchor scan budgets.
