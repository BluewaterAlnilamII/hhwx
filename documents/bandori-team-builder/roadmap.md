...
| `low-memory-polish-hhwx-2026-06-18T06-17-31-085Z.json` | `P02:260` opt-in compatible-row cap-boundary pair probe | bounded gap `382812`, average `9376984`, max `9412868`, `0 failed / 0 timedOut / 0 memoryLimited`, peak `2923 MiB`; typed row state `1.03 MiB`, skipped `6,367,445` incompatible pairs across `30` top rows in `1536ms`, exhausted `29` rows, generated frontier still incomplete |
...
- compatible-row advancement in `low-memory-polish-hhwx-2026-06-18T06-17-31-085Z.json` skips many incompatible pairs cheaply but still does not close the generated frontier: the probe scanned `6.37M` incompatible pairs across only `30` top rows, and `29` of those rows had no compatible generated partner in the full other slot. This changes the next proof question from "pop more pairs" to "prove or skip conflict-saturated rows/groups." A useful next slice should summarize why the top exhausted rows are impossible, likely by shared banned/card-overlap cover or raw row dominance, before trying to promote any pruning.
...
3. Use the closure ledger to choose the next exact-safe cut:
   ...
   - if compact row-frontier exhausts many top rows, add row/group conflict-cover diagnostics before raising timeboxes;
