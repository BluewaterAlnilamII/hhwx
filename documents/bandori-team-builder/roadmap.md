...
3. Add a shadow raw candidate builder:
   ...
   - next raw-builder step: reduce oracle-side rich retention during candidate fill, for example by sampling or delayed hydration rather than keeping every rich candidate. Acceptance for that slice is lower `richCandidateOracleCount` or resident rich count while preserving direct raw primary score/max parity and no false exact.
