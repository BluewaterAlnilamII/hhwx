...
| `low-memory-polish-hhwx-2026-06-18T09-51-35-612Z.json` | `P01:none` raw card-search-index schema smoke | exact, gap `0`, average `7927236`, max `7982835`, peak `955 MiB`; raw mirror fields now include `cardSearchIndex0..4`, mismatch total `0`, and winner raw card-search indices match source candidate indices |
| `low-memory-polish-hhwx-2026-06-18T09-57-18-881Z.json` / `low-memory-polish-hhwx-2026-06-18T10-01-22-734Z.json` | P02 default/no-raw runner variance check around raw schema WIP | both failed with exit `134` / JS heap OOM after about `196s`; the second run was on clean `aa6284f`, so this is not attributable to the raw card-search-index WIP |
| `low-memory-polish-hhwx-2026-06-18T10-09-38-782Z.json` | `P01:none` raw-row exact hydration smoke | exact, gap `0`, average `7927236`, max `7982835`, peak `1202 MiB`; raw row hydration uses `cardSearchIndex0..4` directly, does not rely on `sourceIndex`, and matches object score/average/max/min/card ids with `cardIdMismatchCount = 0` |
| `low-memory-polish-hhwx-2026-06-18T10-21-58-715Z.json` | `P01:none` raw-resident fill readiness profile | exact, gap `0`, average `7927236`, max `7982835`, peak `1084 MiB`; opt-in `rawResidentFill` is no-op (`behaviorChange = false`, `candidateRemoval = false`, `resultAuthoritative = false`) and proves the raw solver winner can be hydrated from raw rows without `sourceIndex` or rich candidate bodies, matching source hydration and raw/object score/average/max/min/card-id parity |
| `low-memory-polish-hhwx-2026-06-18T10-35-23-903Z.json` | `P01:none` guarded raw-resident result release smoke | exact, gap `0`, average `7927236`, max `7982835`, peak `1186 MiB`; opt-in `HHWX_LOW_MEMORY_RAW_RESIDENT_RESULT=1` is installed but safely skips this benchmark path with `skipReason = no-object-result`, `behaviorChange = false`, while raw fill and raw/object parity still match |
| `low-memory-polish-hhwx-2026-06-18T10-46-59-897Z.json` | `P01:none` direct raw-resident result harness | exact, gap `0`, average `7927236`, max `7982835`, peak `1196 MiB`; `rawResidentDirectResultHarness.returnedRawResult = true`, raw solve over `1550` resident rows took `6ms`, raw-row hydration rebuilt max/min/card ids, and object oracle matched score/average/max/min/card ids |
| `low-memory-polish-hhwx-2026-06-18T10-48-50-516Z.json` | `P02:260` default pressure smoke after direct harness slice | failed with exit `134` / Node heap OOM after `187s`; raw-resident flags were unset, so this does not prove a direct-harness regression, but the current branch still lacks a passing post-slice P02 pressure smoke |
| `low-memory-polish-hhwx-2026-06-18T10-56-20-440Z.json` | `P01:none` opt-in raw primary solve | exact, gap `0`, average `7927236`, max `7982835`, peak `1462 MiB`; `rawResidentPrimarySolve.returnedRawResult = true`, rich candidates are `oracle-only`, `richCandidatePrimaryRetainedCount = 0`, and `rawResidentPrimarySolveRelease.richCandidateCountAfterRelease = 0` |
| `low-memory-polish-hhwx-2026-06-18T10-57-58-781Z.json` | `P01:244` opt-in raw primary solve | exact, gap `0`, average `8858388`, max `8913195`, peak `902 MiB`; raw primary solve returned raw result over `4125` oracle candidates, object oracle matched, and rich candidate arrays were released to `0` after return |
| `low-memory-polish-hhwx-2026-06-18T11-05-55-899Z.json` | `P01:none` opt-in raw primary solve with winner-only oracle | exact, gap `0`, average `7927236`, max `7982835`, peak `1187 MiB`; `HHWX_LOW_MEMORY_RAW_RESIDENT_WINNER_ORACLE=1` skips the full object oracle after raw-row hydration, reports `richCandidateRole = winner-hydration-only`, `richCandidateOracleCount = 0`, `winnerHydrationCount = 3`, `objectOracleMode = winner-only-raw-row`, and uses `exactnessBasis = raw-solver-over-resident-rows` |
| `low-memory-polish-hhwx-2026-06-18T11-31-21-079Z.json` | `P01:none` opt-in raw pair-complement upper probe | exact, gap `0`, average `7927236`, max `7982835`, peak `1194 MiB`; raw mirror complete (`1550` rows, `0` mismatch, `0.15 MiB`) but `rawPairComplementUpper.queryCount = 0`, so this small row does not exercise the pair-complement upper path |
| `low-memory-polish-hhwx-2026-06-18T11-35-00-397Z.json` | `P02:260` opt-in raw pair-complement upper probe | failed with exit `134` / Node heap OOM after `172702ms`; this rejects scan-style raw pair-complement upper as a hard-row hot path and points back to compressed frontier/cached raw query or earlier pre-birth pruning |
...
- raw rows now carry `cardSearchIndex0..4` in addition to card ids. `P01:none` artifact `low-memory-polish-hhwx-2026-06-18T09-51-35-612Z.json` proves the schema: mirror mismatch total `0`, result parity stays exact, and the raw winner rows' `cardSearchIndices` match the source candidates. This is the first step toward hydrating from raw rows without retaining rich candidate bodies;
- `P01:none` artifact `low-memory-polish-hhwx-2026-06-18T10-09-38-782Z.json` proves the next step: `raw-row-exact-result` hydration reconstructs the winning cards directly from raw `cardSearchIndex0..4`, calls the normal exact team evaluator only for the three winners, and matches object score, average, max, min, and card ids. `sourceIndex` is now a debug/oracle pointer rather than the only hydration route;
- `P01:none` artifact `low-memory-polish-hhwx-2026-06-18T10-21-58-715Z.json` adds the first raw-resident fill readiness profile. It remains non-authoritative and no-op, but `matchesSourceHydration = true`, `sourceIndexRequiredForWinnerHydration = false`, and `richCandidateRequiredForWinnerHydration = false`; this confirms the next slice can replace small-row winner/result construction with raw rows while retaining the rich path as oracle;
- guarded raw-result release is now wired behind `HHWX_LOW_MEMORY_RAW_RESIDENT_RESULT=1`. `P01:none` artifact `low-memory-polish-hhwx-2026-06-18T10-35-23-903Z.json` proves the guard is non-regressing but also shows the current benchmark path often has no exact-join object result to replace (`skipReason = no-object-result`). Do not claim a release-path memory win until a direct raw-resident fill/oracle harness shows `returnedRawResult = true`;
- direct raw-resident result harness is now wired behind `HHWX_LOW_MEMORY_RAW_RESIDENT_RESULT=1`. `P01:none` artifact `low-memory-polish-hhwx-2026-06-18T10-46-59-897Z.json` proves a small-row raw primary result can be produced from resident rows and verified against an object oracle: `returnedRawResult = true`, `matchesObjectOracle = true`, and output score/average/max/min/card ids match;
- opt-in raw primary solve now uses the direct harness result before object final solve when oracle parity passes. `P01:none` and `P01:244` artifacts `low-memory-polish-hhwx-2026-06-18T10-56-20-440Z.json` / `low-memory-polish-hhwx-2026-06-18T10-57-58-781Z.json` both return raw primary results, keep exact/gap/average/max fields stable, mark rich candidates as `oracle-only`, and release rich candidate arrays to `0` after return;
- opt-in winner-only raw oracle mode is now available behind `HHWX_LOW_MEMORY_RAW_RESIDENT_WINNER_ORACLE=1`. `P01:none` artifact `low-memory-polish-hhwx-2026-06-18T11-05-55-899Z.json` proves the small-row raw solver can be treated as the authoritative result after raw-row winner hydration, without retaining or constructing a full object oracle in that path (`richCandidateOracleCount = 0`, `winnerHydrationCount = 3`, `objectOracleSkipped = true`). This is a raw-resident architecture boundary, not a hard-row memory win;
- opt-in raw pair-complement upper is now wired as a representation probe behind `HHWX_LOW_MEMORY_RAW_PAIR_COMPLEMENT_UPPER=1`, but the first evidence rejects the scan-style version as a mainline route. `P01:none` artifact `low-memory-polish-hhwx-2026-06-18T11-31-21-079Z.json` keeps result fields stable but does not issue pair-complement queries (`queryCount = 0`); `P02:260` artifact `low-memory-polish-hhwx-2026-06-18T11-35-00-397Z.json` OOMs after `172702ms`. Do not promote raw pair-complement scan; future raw proof work needs compressed row-frontier/cached records or must move earlier to candidate-birth pruning;
- P02 runner variance remains high: WIP and clean no-raw controls around `09:57` / `10:01` OOMed, and the post-slice default P02 pressure smoke `low-memory-polish-hhwx-2026-06-18T10-48-50-516Z.json` also OOMed with raw-resident flags unset. Do not claim a hard-row memory improvement from this slice; re-establish a passing P02 pressure control before hard-row release claims;
- conclusion: the raw-row representation is not the memory bottleneck, sorted raw rows can be made order-compatible with the object solver, and raw winner indices can be late-hydrated into full HHWX result fields. The small-row path can now return a raw primary result and can skip the object oracle in winner-only mode. The remaining handoff risk is candidate-birth/storage lifecycle: next move hard-row candidate fill/frontier helpers toward compressed raw-index residency or pre-materialization proof gates; do not spend more effort on scan-style raw pair-complement replacement.
...
3. Add a shadow raw candidate builder:
   ...
   - raw rows now include `cardSearchIndex0..4`, and P01 proves raw-row exact hydration can reconstruct selected cards from `slot.searchCards` without `sourceIndex`.
   - raw-resident fill readiness now proves the raw solver winner can be hydrated from raw rows without `sourceIndex` or rich candidate bodies, while still matching source hydration and result parity.
   - guarded raw-result release is wired, but retained P01 smokes skip with `no-object-result`; it is a safety boundary, not a memory win.
   - direct raw-resident result harness now gives the missing small-row evidence: `returnedRawResult = true` with object oracle parity on `P01:none`.
   - opt-in raw primary solve now returns raw-row results on `P01:none` and `P01:244`, with rich candidates marked as oracle-only and released to zero after return.
   - winner-only raw oracle now proves the raw primary path can skip the full object oracle on a small row after winner hydration (`richCandidateOracleCount = 0`, `winnerHydrationCount = 3`), while still preserving average/max output fields.
   - raw pair-complement upper scan was tested as an opt-in representation probe and rejected for hard rows: P01 did not exercise the query path, while P02 OOMed. Keep it only as a negative diagnostic and pursue compressed row-frontier/cached raw proof or earlier pre-materialization pruning.
   - next raw-builder step: make the hard-row fill/frontier lifecycle consume raw-index rows as the primary resident representation, then add pre-materialization legality/upper/dominance gates before rich candidate birth. Acceptance for that slice is lower hard-row rich resident count or lower `P02:260` materialized rich birth while preserving direct raw primary score/max parity and no false exact.
