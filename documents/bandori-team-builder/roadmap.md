...
- P02 runner variance is currently high: both a WIP no-raw control (`low-memory-polish-hhwx-2026-06-18T09-57-18-881Z.json`) and a clean `aa6284f` no-raw control (`low-memory-polish-hhwx-2026-06-18T10-01-22-734Z.json`) OOMed. Do not attribute those failures to the raw card-search-index schema; use P01 parity plus typecheck for this schema slice, and re-establish P02 pressure stability before claiming a hard-row memory improvement;
- conclusion: the raw-row representation is not the memory bottleneck, sorted raw rows can be made order-compatible with the object solver, and raw winner indices can be late-hydrated into full HHWX result fields. The remaining handoff risk is now implementation structure, not basic result reconstruction: equal-average tie handling stays guarded, and the next architecture slice should move candidate fill itself to a raw-resident/oracle split so that a real `returnedRawResult = true` path exists before attempting hard-row release.
...
3. Add a shadow raw candidate builder:
   ...
   - next raw-builder step: implement an opt-in small-row candidate-fill split where raw rows are the primary resident storage and rich candidates are retained only for oracle comparison. Acceptance for that slice is `returnedRawResult = true` on a direct harness plus unchanged P01 focused scores.
