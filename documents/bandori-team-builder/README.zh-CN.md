...
两个计算器都从 HHWX 游戏档案和 Bandori 主数据／谱面出发，共享组曲规范化、参数和精确区间计分基础，并在同一个 Rust/WASM 包内交付。单曲加入加权站位期望及其活动规则，三曲组曲模型保持不变。

精确搜索的保证有明确前提：`exact` 证明在已校验的规范输入和文档规定的计分规则内最优；`incomplete` 只报告截至停止时找到的最好方案（如果有）。这一保证不能独立认证上游数据或尚未建模的游戏机制。
...
所选档案按当前登录的 HHWX 账号在本地记忆，记录档案来源与 ID。重新加载档案列表时优先保留仍有效的当前选择，再尝试保存的选择，最后回退到首个云端或本地档案。不会仅凭存储记录恢复已经不可访问的档案。

演出类型和谱面难度只记忆用户主动选择。活动限制和缺少相应难度的谱面只改变当前实际生效的输入，不覆盖这些偏好；组曲的三个歌曲槽位分别保留各自的首选难度。界面、谱面预加载和搜索请求使用同一份实际生效值。

协力技能条件开关默认开启，已明确保存的开关选择会保留。搜索目标、结果数量、时限、结果页的火罐／CP／名次／胜负以及临时卡不持久化。这些前端偏好规则不改变计分或搜索算法。

运行 `node --test tests/bandori-team-builder-preferences.test.mjs` 可执行偏好恢复和异步加载竞争的专项检查。
