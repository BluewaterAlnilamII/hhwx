...
Every score cut is strict against an exact feasible cutoff: normally the global incumbent, and inside a local join possibly the best candidate already scored for that fixed pair. Equality remains because the deterministic tie representative may improve. An unavailable finite proof bound becomes infinity or disables that optimization, so exhaustive search may still finish `exact`. During search, detectable exact-scoring/input arithmetic failures, controlled allocation failures, timeout, cancellation, counter overflow, and internal inconsistency fail closed as `incomplete`, optionally with diagnostic `bestSoFar` and discovered solutions. Hydration runs after search; a hydration score disagreement rejects the WebAssembly call instead of returning either terminal search status, so an inconsistent `exact` result is not delivered. If the process, worker, or browser is terminated outright, no final outcome can be promised, but it cannot be reported as `exact`. There is no score-gap success state.
...
- tiny exhaustive search against an independent exhaustive reference under different memory budgets and configurations, including a six-character case where one eligible card remains unused and different teams omit different characters;
