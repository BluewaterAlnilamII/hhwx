...
For local development, unpacked JP resource files may be staged directly under `public/local/chart-simulator/`. This ignored directory has no region or version layer, generated catalog, recipe, or content-addressing contract. Merely staging a file does not authorize the simulator to load it; each presentation capability remains disabled until its native behavior is verified.
