...
The user-approved vertical-beam and outward-growing Directional-terminal placement compensations are renderer-wide. Persona maps its equivalent `line1`, finger `/slash`, and main `/notes` nodes into the same rules; it does not introduce skin-specific numeric offsets.
