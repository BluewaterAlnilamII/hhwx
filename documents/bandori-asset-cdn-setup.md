# HHWX Production CDN Runbook

> 本文档记录 HHWX 当前生产部署的 CDN/R2 运维方案。它不是公共自部署模板，不是第三方素材授权声明，也不要求或允许自部署者复用 HHWX 的生产 CDN、存储桶或凭据。开源自部署时请替换为自己的 CDN、存储桶和凭据，并自行确认对第三方游戏素材的缓存、镜像和展示行为是否合规。
