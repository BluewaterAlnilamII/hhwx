...
其中没有候选生成、卡池枚举、区域道具搜索、排序、剪枝、证明协议、时间／内存预算、取消或部分结果语义。接近 2,000 张卡的卡池只属于未来的困难验收场景，不是早期测试、默认输入或隐含内存目标。搜索架构必须等到另一次独立设计审查后才开始。
...
15 张已选卡必须都由档案拥有，同一物理卡在整套组曲里只能出现一次；按照规范化 Rust 契约，每支固定队还必须由五个不同角色组成。完整 profile 仍解码排除标记，但它属于搜索偏好，不改变已经显式给定的固定计分输入。缺失选中 master 行、档案压缩损坏、引用无效、十进制不规范、foundation 自有 envelope 出现未知字段或 schema 版本不支持，都会以稳定错误码和字段路径失败；原始 master 行中未使用的字段仍被容忍。它们是输入错误，绝不是“无解”。本检查点没有临时卡或搜索候选。
...
本检查点到此停止。在加入任何 roster、candidate layout、剪枝规则、缓存、支配关系、并行搜索、证明证书、内存预算或生产结果协议之前，必须把搜索架构作为独立阶段重新讨论和审查。当前基础设施既不推荐、也不暗中限制未来搜索设计。
