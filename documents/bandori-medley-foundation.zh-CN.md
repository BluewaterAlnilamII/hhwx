...
- 按角色组织卡池，在完整五卡候选产生之前，对部分队伍所代表的整批组合剪枝。核心判断是整套三曲目标的安全上界，不是候选生成后的配对优化。
...
- 区域配置和各搜索阶段的数据只在仍有用途时常驻；只有通过剪枝的完整队伍才精确计分并写入基于索引的紧凑候选，完整输出对象只为最终保留的少量结果还原。
- 无法证明覆盖全部可能补全方案的上界不得剪枝。禁止候选上限、随机保留、近似比较和虚假的 exact 成功；在没有上界命中的小输入上，搜索仍应完整穷举。

对 calc 的反向研究只用于说明剪枝应放在哪里、紧凑数据应存活多久；它不定义 HHWX 的计分、候选布局、替代关系或证明语义。具体上界模型、部分队伍表示、最终三队不重复组合、存储布局和并行策略仍须在实施前作为一套完整方案统一审查。
...
本检查点仍停在搜索实现之前。上文锁定的产品规则与暂定方向尚未选定具体上界算法、候选布局、缓存、替代关系、最终组合、分区方式或并行策略；这些仍须按上文所述作为一套完整架构统一审查。
