...
const scenarios = {
  ...
  "p03-244-trace-300": {
    description: "Trace P03:244 all-scope at 300s.",
    reportKind: "focus",
    recordOnly: true,
    maxElapsedMs: 300000,
    env: {
  ...
  "p10-244-soft700-trace-300": {
    description: "Trace P10:244 at 300s with a 700k exact candidate soft limit.",
    reportKind: "focus",
    recordOnly: true,
    maxElapsedMs: 300000,
    env: {
      ...
      HHWX_REAL_PROFILE_MATRIX_LOCKED_SCOPES: "0",
      HHWX_REAL_PROFILE_DURATION_MS: "300000",
      HHWX_REAL_PROFILE_OPTIMIZATION_JSON: "{\"debugConfigurationTrace\":true,\"exactCandidateSoftLimit\":700000}",
    },
    cases: [
      { profileLabel: "P10", eventKey: "244" },
    ],
  ...
  "p10-244-no-same-coarse-skip-trace-300": {
    description: "Diagnostic trace P10:244 at 300s with same-coarse tight root skip disabled.",
    reportKind: "focus",
    recordOnly: true,
    maxElapsedMs: 300000,
    env: {
      ...
      HHWX_REAL_PROFILE_MATRIX_LOCKED_SCOPES: "0",
      HHWX_REAL_PROFILE_DURATION_MS: "300000",
      HHWX_REAL_PROFILE_OPTIMIZATION_JSON: "{\"debugConfigurationTrace\":true,\"disableSameCoarseTightRootSkip\":true}",
    },
    cases: [
      { profileLabel: "P10", eventKey: "244" },
    ],
  },
  "p10-244-upper-trace-300": {
    description: "Trace P10:244 at 300s with additional capacity upper options enabled.",
    reportKind: "focus",
    recordOnly: true,
    maxElapsedMs: 300000,
    env: {
...
function toFocusMarkdown(report) {
  return [
    ...
    "| case | baseline | exact | elapsed ms | gap | abort reason | abort slot | soft limit | candidates | third fallback | extended hits | extended fallback | fallback words | guarded limit | anchor trigger | anchor skip | anchor high pairs | cheap count | cheap processed | cheap gap | cheap elapsed | cheap split attempts | cheap split states | cheap split abort | probe hit | probe processed | probe score | probe elapsed | anchor processed | anchor gap | anchor elapsed | anchor timebox | staged limit | staged gap | small retry | retry timebox | same-coarse mem skip | top frontier | peak MiB | status |",
    "| --- | --- | --- | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | ---: | --- |",
    ...report.rows.map((row) => [
      row.caseKey,
      ...
      row.result.exactCandidateJoinAnchorFrontierProofTriggerCount ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierProofSkipReason ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierProofHighPairRecordUpperCount ?? "",
      row.result.exactCandidateJoinAnchorFrontierCheapUpperCount ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierCheapUpperProcessedAnchorCount ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierCheapUpperResidualGap ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierCheapUpperElapsedMs ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierCheapUpperSplitAttemptCount ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierCheapUpperSplitStateCount ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierCheapUpperSplitAbortReason ?? "",
      row.result.exactCandidateJoinAnchorFrontierImprovementProbeHitCount ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierImprovementProbeProcessedAnchorCount ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierImprovementProbeScore ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierImprovementProbeElapsedMs ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierProofProcessedAnchorCount ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierProofResidualGap ?? "",
      row.result.exactCandidateJoinLastAnchorFrontierProofElapsedMs ?? "",
      row.result.exactCandidateJoinAnchorFrontierProofTimeboxCount ?? "",
